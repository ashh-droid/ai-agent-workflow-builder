...
export function Providers({ children }: { children: ReactNode }) {
  ...
  const wsClient = useMemo<WsClient>(
    () =>
      createWsClient({
  ...
  const urql = useMemo(
    () =>
      createUrqlClient({
        url: graphqlUrl(),
        // Nhost's hosted GraphQL gateway rejects the GET/persisted-query form
        // that current urql may choose for short query operations. Force the
        // standard POST transport that Hasura/Nhost accepts consistently.
  ...
  return (
    <AuthContext.Provider value={{ nhost, session, isLoading }}>
