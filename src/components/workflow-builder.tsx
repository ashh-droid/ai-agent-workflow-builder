...
import { ArrowDown, ArrowUp, Bot, Braces, Database, GitBranch, Globe2, LockKeyhole, Plus, Save, Send, ShieldCheck, Trash2, Webhook, Zap } from "lucide-react";
...
function stepLabel(type: StepType) {
  return stepTypes.find((item) => item.type === type)?.label.replace(" · owner", "") ?? type;
}
...
export function WorkflowBuilder({ orgId, role, workflow, onSave }: { orgId: string; role: OrgRole; workflow: Workflow | null; onSave: (workflow: Workflow) => Promise<void>; }) {
  ...
  function addStep(type: StepType) { if (restricted.has(type) && role !== "owner") return; setDraft({ ...draft, steps: [...draft.steps, { step_order: draft.steps.length + 1, step_type: type, name: stepLabel(type), config: defaultConfig(type) }] }); }
  ...
  return <section className="builder-panel">
    <div className="section-heading"><div><p className="eyebrow">WORKFLOW BUILDER</p><h2>{draft.id ? "Edit workflow" : "New workflow"}</h2></div><div className="row gap-sm builder-actions">{role === "owner" && <button className="secondary reviewer-button" onClick={() => setDraft(reviewerDemo(orgId))}><Zap size={15}/>Load reviewer demo</button>}{role !== "viewer" && <button className="primary" onClick={save} disabled={saving}><Save size={15}/>{saving ? "Saving…" : "Save"}</button>}</div></div>
    ...
    <div className="trigger-box"><div className="subsection-title"><h3>Triggers</h3><span>How this workflow can start</span></div><div className="trigger-grid"><TriggerToggle label="Manual" icon={<Zap size={15}/>} checked={!!trigger("manual")} disabled={role === "viewer"} onChange={(value) => toggleTrigger("manual", value)}/><TriggerToggle label="Webhook · owner" icon={<Webhook size={15}/>} checked={!!trigger("webhook")} disabled={role !== "owner"} onChange={(value) => toggleTrigger("webhook", value)}/><TriggerToggle label="Scheduled" icon={<Braces size={15}/>} checked={!!trigger("scheduled")} disabled={role === "viewer"} onChange={(value) => toggleTrigger("scheduled", value)}/><TriggerToggle label="Database event" icon={<Database size={15}/>} checked={!!trigger("db_event")} disabled={role === "viewer"} onChange={(value) => toggleTrigger("db_event", value)}/></div>{trigger("webhook") && role === "owner" && <label>Webhook secret<input type="password" placeholder={trigger("webhook")?.id ? "Leave blank to keep existing secret" : "At least 12 characters"} value={String(trigger("webhook")?.config.secret ?? "")} onChange={(e) => patchTrigger("webhook", { secret: e.target.value })}/></label>}{trigger("scheduled") && <label>Cron (UTC)<input value={String(trigger("scheduled")?.config.cron ?? "")} disabled={role === "viewer"} onChange={(e) => patchTrigger("scheduled", { cron: e.target.value })}/></label>}{trigger("db_event") && <label>Event name<input value={String(trigger("db_event")?.config.event_name ?? "")} disabled={role === "viewer"} onChange={(e) => patchTrigger("db_event", { event_name: e.target.value })}/></label>}</div>
    <div className="steps-header"><div><h3>Steps</h3><span>Ordered execution graph</span></div><span className="node-count">{draft.steps.length} nodes</span></div><div className="step-list">{draft.steps.map((step,index)=><article className={`builder-step step-${step.step_type}`} key={step.id ?? `${step.step_type}-${index}`}><div className="step-order">{step.step_order}</div><div className="step-body"><div className="step-toolbar"><div className="step-type"><span className="step-icon"><StepIcon type={step.step_type}/></span><div><strong>{stepLabel(step.step_type)}</strong><small>{step.step_type}</small></div>{restricted.has(step.step_type)&&<span className="owner-pill"><LockKeyhole size={12}/>owner</span>}</div><div className="row gap-xs"><button className="icon-btn" aria-label="Move step up" disabled={!canReorder||index===0||role==="viewer"} onClick={()=>move(index,-1)}><ArrowUp size={15}/></button><button className="icon-btn" aria-label="Move step down" disabled={!canReorder||index===draft.steps.length-1||role==="viewer"} onClick={()=>move(index,1)}><ArrowDown size={15}/></button><button className="icon-btn danger" aria-label="Remove step" disabled={draft.steps.length===1||role==="viewer"||(restricted.has(step.step_type)&&role!=="owner")} onClick={()=>removeStep(index)}><Trash2 size={15}/></button></div></div><label>Step name<input value={step.name} disabled={role==="viewer"||(restricted.has(step.step_type)&&role!=="owner")} onChange={(e)=>patchStep(index,{name:e.target.value})}/></label><StepConfig step={step} index={index} role={role} total={draft.steps.length} patch={patchConfig}/></div></article>)}</div>
    {role !== "viewer" && <div className="add-step-row">{stepTypes.map((item)=><button key={item.type} className={`chip step-chip step-${item.type}`} disabled={restricted.has(item.type)&&role!=="owner"} onClick={()=>addStep(item.type)}><span className="step-chip-icon"><StepIcon type={item.type}/></span><Plus size={12}/>{item.label}</button>)}</div>}
  </section>;
}
...
function StepIcon({type}:{type:StepType}) { if(type==="llm_call")return <Bot size={16}/>; if(type==="http_request")return <Globe2 size={16}/>; if(type==="conditional_branch")return <GitBranch size={16}/>; if(type==="approval_gate")return <ShieldCheck size={16}/>; if(type==="db_write")return <Database size={16}/>; if(type==="notify")return <Send size={16}/>; return <Braces size={16}/>; }
function StepConfig({step,index,role,total,patch}:{step:WorkflowStep;index:number;role:OrgRole;total:number;patch:(index:number,key:string,value:unknown)=>void}) {
  ...
  if(step.step_type==="llm_call")return <div className="grid-two"><label>Model<input disabled={disabled} value={String(step.config.model??"gemini-3.5-flash-lite")} onChange={(e)=>patch(index,"model",e.target.value)}/></label><label>Temperature<input disabled={disabled} type="number" min="0" max="2" step="0.1" value={Number(step.config.temperature??0)} onChange={(e)=>patch(index,"temperature",Number(e.target.value))}/></label><label className="span-two">Prompt<textarea disabled={disabled} value={String(step.config.prompt_template??"")} onChange={(e)=>patch(index,"prompt_template",e.target.value)}/></label></div>;
  ...
  if(step.step_type==="conditional_branch")return <div className="grid-four"><label>Output path<input disabled={disabled} value={String(step.config.path??"text")} onChange={(e)=>patch(index,"path",e.target.value)}/></label><label>Match value<input disabled={disabled} value={String(step.config.value??"POSITIVE")} onChange={(e)=>patch(index,"value",e.target.value)}/></label><label>True → step<select disabled={disabled} value={Number(step.config.true_step_order??Math.min(total,step.step_order+1))} onChange={(e)=>patch(index,"true_step_order",Number(e.target.value))}>{Array.from({length:Math.max(0,total-step.step_order)},(_,i)=>step.step_order+i+1).map((n)=><option key={n} value={n}>{n}</option>)}</select></label><label>False → step<select disabled={disabled} value={Number(step.config.false_step_order??total)} onChange={(e)=>patch(index,"false_step_order",Number(e.target.value))}>{Array.from({length:Math.max(0,total-step.step_order)},(_,i)=>step.step_order+i+1).map((n)=><option key={n} value={n}>{n}</option>)}</select></label></div>;
